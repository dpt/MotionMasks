...
mmerror_t stream_op(stream_t *s, stream_opcode_t opcode, void *opaque)
{
  if (!s->op)
    return mmerror_STREAM_UNKNOWN_OP;

  return s->op(s, opcode, opaque);
}

mmerror_t stream_seek(stream_t *s, stream_size_t pos)
{
  if (!s->seek)
    return mmerror_STREAM_CANT_SEEK;
